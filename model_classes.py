...
import numpy as np
from utils import probs_to_logits, calculate_transition_probability
...
class Noised_Multinomial_VAE(nn.Module):
    def __init__(self, input_dim, hidden_dim, hidden_amount, latent_dim, n_trials, noise_matrix, tau=0.01, device='cpu'):
        super(Noised_Multinomial_VAE, self).__init__()
        assert np.shape(noise_matrix) == (latent_dim, latent_dim)
        self.n_trials = n_trials  # Number of trials for the Binomial distribution
        self.tau = tau
        self.latent_dim = latent_dim
        if latent_dim == 1:
        ...
        # This step assumes a small number of trials and categories for feasibility
        from itertools import product
        count_vectors_list = [list(count) for count in product(range(self.n_trials + 1), repeat=latent_dim)
             if sum(count) == self.n_trials]
        self.count_vectors = torch.tensor(
            count_vectors_list,
            dtype=torch.float,
        ...
        for combi in count_vectors_list:
            probability_matrix.append(calculate_transition_probability(combi, noise_matrix, n_trials, count_vectors_list))

        self.probability_matrix = torch.tensor(probability_matrix, dtype=torch.float, device=device)
    ...
    def multinomial(self, probs):
        ...
        noised_channel = torch.matmul(probabilities, self.probability_matrix)
        return noised_channel
